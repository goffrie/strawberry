...
async fn main() {
    ...
    for &word1 in words::FRUITS {
        for &word2 in words::FRUITS {
            ...
            let request = serde_json::to_string(&ListReq {
                version: 0,
                room: &room_name,
            })
            .unwrap();
            let response = client
                .request(hyper::Request::post(&url).body(request.into()).unwrap())
                .await
                .expect("request failed");
            let response_data = hyper::body::aggregate(response.into_body())
                .await
                .expect("read failed");
            let reply: Option<ListReply> =
                serde_json::from_reader(response_data.reader()).expect("json failed");
